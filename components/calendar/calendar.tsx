...
function Calendar(): JSX.Element {
  ...
  const datesMay = Array.from({ length: 31 }, (_, i) => new Date(`05.${i + 1}.2024`));
  const datesJune = Array.from({ length: 30 }, (_, i) => new Date(`06.${i + 1}.2024`));
  const datesJuly = Array.from({ length: 31 }, (_, i) => new Date(`07.${i + 1}.2024`));
  const datesAugust = Array.from({ length: 31 }, (_, i) => new Date(`08.${i + 1}.2024`));
